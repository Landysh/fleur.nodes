...
public class RectangleGateAnnotation extends XYBoxAnnotation implements XYGateAnnotation {
  ...
  public XYGateAnnotation translate(double dx, double dy) {
    return new RectangleGateAnnotation(subsetName, domainAxisName, rangeAxisName, x0 + dx, y0 + dy,
        x1 + dx, y1 + dy, LookAndFeel.SELECTED_STROKE, LookAndFeel.SELECTED_GATE_COLOR);
  }
  ...
  public boolean matchesVertex(Point2D v, double xHandleSize, double yHandleSize) {
    ...
    if ((xMin <= x0 && x0 <= xMax) || (xMin <= x1 && x1 <= xMax)) {
      xMatches = true;
    }
    if ((yMin <= y0 && y0 <= yMax) || (yMin <= y1 && y1 <= yMax)) {
      yMatches = true;
    }
