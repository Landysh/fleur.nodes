...
public class SPTree {
  ...
  // Axis-aligned bounding box stored as a center with half-dimensions to represent the boundaries
  // of this quad tree
  Cell boundary;

  // Indices in this space-partitioning tree node, corresponding center-of-mass, and list of all
  // children
  double[] data;
  ...
  // Constructor for SPTree with particular size and parent -- build the tree, too!
  SPTree(SPTree inp_parent, int D, double[] inp_data, int N, double[] inp_corner,
      double[] inp_width) {
    init(inp_parent, D, inp_data, inp_corner, inp_width);
    fill(N);
  }
  ...
  int getDepth() {
    ...
    int depth = 0;
    for (int i = 0; i < no_children; i++)
      depth = max(depth, children[i].getDepth());
    return 1 + depth;
  }
  ...
  void print() {
    ...
    if (is_leaf) {
      ...
      for (int d = 0; d < dimension; d++)
        System.out.printf("%f, ", center_of_mass[d]);
