...
public class ParallelBHTsne extends BHTSne {
  ...
  class RecursiveGradientCalculator extends RecursiveAction {
    ...
    protected void compute() {
      if ((endRow - startRow) <= limit) {
        ...
        int endDoc2 = endRow;
        invokeAll(
            new RecursiveGradientCalculator(tree, neg_f, theta, sum_Q, startDoc1, endDoc1, limit),
  ...
  class ParallelGradientCalculator implements Callable<Double> {
    ...
    public ParallelGradientCalculator(ParallelSPTree tree, double[][] neg_f, double theta, int row,
        int ll) {
      this.limit = ll;
      this.row = row;
      this.tree = tree;
  ...
  class RecursiveGradientUpdater extends RecursiveAction {
    ...
    public RecursiveGradientUpdater(int n, int no_dims, double[] Y, double momentum, double eta,
    ...
    protected void compute() {
      if ((endIdx - startIdx) <= limit) {
        ...
        int endIdx2 = endIdx;
        invokeAll(
            new RecursiveGradientUpdater(N, no_dims, Y, momentum, eta, dY, uY, gains, startIdx1,
                endIdx1, limit),
            new RecursiveGradientUpdater(N, no_dims, Y, momentum, eta, dY, uY, gains, endIdx1,
                endIdx2, limit));
      }
    }
  }
  ...
  void updateGradient(int N, int no_dims, double[] Y, double momentum, double eta, double[] dY,
    ...
    RecursiveGradientUpdater dslr = new RecursiveGradientUpdater(N, no_dims, Y, momentum, eta, dY,
        uY, gains, 0, N * no_dims, N / (Runtime.getRuntime().availableProcessors() * 10));
    gradientPool.invoke(dslr);
  }

  // Compute gradient of the t-SNE cost function (using Barnes-Hut algorithm)
  @Override
  void computeGradient(double[] P, int[] inp_row_P, int[] inp_col_P, double[] inp_val_P, double[] Y,
    ...
    double sum_Q = 0;
    // RecursiveGradientCalculator dslr = new RecursiveGradientCalculator(tree, neg_f, theta, sum_Q,
    // 0, N, 20);
    // gradientPool.invoke(dslr);
  ...
  void computeGaussianPerplexity(double[] X, int N, int D, int[] _row_P, int[] _col_P,
    ...
    List<Future<ParallelVpTree<DataPoint>.ParallelTreeNode.TreeSearchResult>> results =
        tree.searchMultiple(tree, obj_X, K + 1);
