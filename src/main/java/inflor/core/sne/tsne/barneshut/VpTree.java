...
public class VpTree<StorageType> {
  ...
  public Node buildFromPoints(int lower, int upper) {
    ...
    if (upper - lower > 1) { // if we did not arrive at leaf yet
      ...
      nth_element(_items, lower + 1, median, upper,
          new DistanceComparator(_items[lower], distance));
  ...
  class Node {
    ...
    double search(Node node, DataPoint target, int k, Queue<HeapItem> heap, double _tau) {
      if (node == null)
        return _tau; // indicates that we're done here
      ...
      if (dist < _tau) {
        if (heap.size() == k)
          heap.remove(); // remove furthest node from result list (if we already have k results)
      ...
      if (dist < node.threshold) {
        if (dist - _tau <= node.threshold) { // if there can still be neighbors inside the ball,
                                             // recursively search left child first
          _tau = search(node.left, target, k, heap, _tau);
        }

        if (dist + _tau >= node.threshold) { // if there can still be neighbors outside the ball,
