...
/**
 * This is the model implementation of SummaryStatistics. Extract basic summary
 * statistics from a set of FCS Files.
 *
 * @author Landysh
 */
	...
	private DataTableSpec createSpec(DataTableSpec inSpec) {
		...
		for (final String marker : m_settings.getResponseMarkersModel().getStringArrayValue()) {
			for (final String stat : m_settings.getSelectedStatsModel().getStringArrayValue()) {
				final DataColumnSpec spec = new DataColumnSpecCreator(stat + " " + marker, DoubleCell.TYPE)
						.createSpec();
