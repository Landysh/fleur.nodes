...
public class UpdateTransformPanelWorker extends SwingWorker<ArrayList<ChartPanel>, String> {
	...
	public UpdateTransformPanelWorker(JProgressBar progressBar, JPanel transformPanel, TransformNodeSettings m_Settings,
			ArrayList<FCSFrame> dataSet) {
	...
	private Map<String, FCSDimension> createDataModel(String parametername, ArrayList<FCSFrame> dataSet) 
	{
		HashMap<String, FCSDimension> dataModel = new HashMap<>();
		for (FCSFrame dataFrame: dataSet){
			dataModel.put(dataFrame.getPrefferedName(), FCSUtils.findCompatibleDimension(dataFrame, parametername));
		}
		return dataModel;
	}
