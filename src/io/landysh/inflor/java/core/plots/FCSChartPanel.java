...
public class FCSChartPanel extends ChartPanel {
	/**
	 * Extends the JFreeChart ChartPanel to provide ability to persists annotations
	 * and map them to gates.
	 */
	...
	public FCSChartPanel(JFreeChart chart, FCSFrame data) {
		...
		getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("DELETE"), DELETE_ANNOTATIONS_KEY);
	...
	public void udpateAnnotation(XYGateAnnotation priorAnnotation, XYGateAnnotation updatedAnnotation) {
	...
	public void createGateAnnotation(XYGateAnnotation annotation) {
		...
		XYTextAnnotation label = new XYTextAnnotation(annotation.getSubsetName() + ": " + result, position[0], position[1]);
	...
	public void setSelectAnnotations(Point2D p) {
		...
		Map<Boolean, List<XYGateAnnotation>> gateSelection = gateAnnotations.keySet()
				.stream()
				.collect(Collectors.partitioningBy(annotation -> annotation.containsPoint(p)));
		
		selectedAnnotations = gateSelection.get(new Boolean(true))
				.stream()
				.map(annotation -> updateSelectionStatus(annotation, true))
	...
	private XYGateAnnotation updateSelectionStatus(XYGateAnnotation priorAnnotation, boolean markSelected) {
	...
	public void translateSelectedAnnotations(double dx, double dy) {
		if (selectedAnnotations!=null&&selectedAnnotations.size()>=1){
			List<XYGateAnnotation> translatedAnnoations = selectedAnnotations
				.stream()
				.map(annotation -> moveAnnotation(annotation, dx, dy))
				.collect(Collectors.toList());
	...
	public boolean hasGatesAtPoint(Point2D v) {
		List<XYGateAnnotation> matchingVertices = selectedAnnotations
			.stream()
			.filter(annotation -> annotation.matchesVertex(v, xHandleSize, yHandleSize))
			.collect(Collectors.toList());
		if (matchingVertices.size()>=1){return true;} 
		return false;
	}

	public void adjustGatesAtPoint(Point2D v, double dx, double dy) {
		if (selectedAnnotations!=null&&selectedAnnotations.size()>=1){
			List<XYGateAnnotation> adjustedAnnotrations = selectedAnnotations
				.stream()
	...
	private XYGateAnnotation updateGateVertex(XYGateAnnotation oldAnn, Point2D v, double xHandleSize, double yHandleSize, double dx, double dy) {
	...
	public String getDomainAxisName() {
		return this.getChart().getXYPlot().getDomainAxis().getLabel();
	}
