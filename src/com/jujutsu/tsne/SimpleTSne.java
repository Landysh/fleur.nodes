  ...
  public double[][] tsne(double[][] X, int no_dims, int initial_dims, double perplexity,
    ...
    for (int iter = 0; iter < max_iter; iter++) {
      ...
      final double[][] num = mo.scalarInverse(mo.scalarPlus(mo.addRowVector(
          mo.transpose(mo.addRowVector(mo.scalarMult(mo.times(Y, mo.transpose(Y)), -2), sum_Y)),
          sum_Y), 1));
      ...
      gains = mo.plus(
          mo.scalarMultiply(mo.scalarPlus(gains, .2),
              mo.abs(mo.negate(mo.equal(mo.biggerThan(dY, 0.0), mo.biggerThan(iY, 0.0))))),
          mo.scalarMultiply(mo.scalarMult(gains, .8),
              mo.abs(mo.equal(mo.biggerThan(dY, 0.0), mo.biggerThan(iY, 0.0)))));
