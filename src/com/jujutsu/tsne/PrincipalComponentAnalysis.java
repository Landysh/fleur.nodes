 ...
 * (e.g. face images) which can then be used to test for membership. PCA works by converting the set
 * of data to a new basis that is a subspace of the original set. The subspace is selected to
 * maximize information.
 * </p>
 * <p>
 * PCA is typically derived as an eigenvalue problem. However in this implementation
 * {@link org.ejml.interfaces.decomposition.SingularValueDecomposition SVD} is used instead because
 * it will produce a more numerically stable solution. Computation using EVD requires explicitly
 * computing the variance of each sample set. The variance is computed by squaring the residual,
...
public class PrincipalComponentAnalysis {
  ...
  public void computeBasis(int numComponents) {
    ...
    final SingularValueDecomposition<DenseMatrix64F> svd =
  ...
  /**
   * <p>
   * The membership error for a sample. If the error is less than a threshold then it can be
   * considered a member. The threshold's value depends on the data set.
   * </p>
   * <p>
   * The error is computed by projecting the sample into eigenspace then projecting it back into
   * sample space and
   * </p>
   * 
